...
dat %>%
  ...
  addCircles(~longitude, ~latitude, color = ~pal(dat$`price group`), weight = 3, opacity = 0.7,popup = pop, group = "Restaurants") %>%
  addMarkers(lat=dat$latitude, lng=dat$longitude,
  clusterOptions = markerClusterOptions(), popup=pop) -> map_nolayer
map_nolayer
```
